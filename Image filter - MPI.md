...
+ Rank 0 will read the image data, and broadcast the image dimensions to all ranks. The paralelization will split the image vertically over the available processes. The local image buffer on each process includes two additional rows, one at the bottom, and one at the top, to work as guard cells. 
...
Use the provided template to write your code. The template already includes the file read/write, histogram and filter routines, as well as MPI initialization and cleanup. It also includes placeholders for you to place your code. __Do not change the structure of the code__.
...
Both the report and the source file must clearly identify the two members of the group, including name and student number. Please submit both files to the e-learning (Moodle) platform in the section made available for this purpose (Avaliação → Projecto 2 - MPI). 
